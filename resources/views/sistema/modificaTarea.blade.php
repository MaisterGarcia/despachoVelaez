					...
					<div class="container col-12">
						<div class="row">
							<div class="form-group col-6">
								...
								Clave Tarea: <input type = 'text' name = 'id_Tarea' value="{{$infom->id_Tarea}}" readonly = 'readonly' class="form-control">
							</div>
							<div class="form-group col-6">
								@if($errors->first('NomTarea'))
								<i> {{ $errors->first('NomTarea') }} </i> 
								@endif	<br>
								Nombre de Tarea a Realizar: <input type = 'text' name = 'NomTarea' value="{{$infom->NomTarea}}" class="form-control">
							</div>
							<div class="col-md-6 ">
								...
								<div class="well well-sm">
									<div class='input-group date' id='divMiCalendario'>
										Introduzca Fecha de Limite de Realización de Tarea:<input type='text' id="txtFecha" class="form-control"  readonly name = 'FechaLimite' value="{{$infom->FechaLimite}}"/>
						...
						@endif
								<div class="well well-sm">
									<div class='input-group date' id='divMiCalendario2'>
										Introduzca Fecha de Finalización de Tarea:<input type='text' id="txtFecha" class="form-control"  readonly name = 'FechaFin' value="{{$infom->FechaFin}}"/>
										<span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span>
									</span>
								</div>	
							</div>
						...
						<div class="form-group col-6">
							...
							<select name = 'num_folio' class="form-control">
								<option value = '{{$num_folio}}'>{{$abogados->NomAbogado.' '.$abogados->AppAbogado.' '.$abogados->ApmAbogado}}</option>
								@foreach($todosdemas as $ab)
							...
							<select name = 'id_EstTar' class="form-control">
								<option value ="{{$id_EstTar}}">{{$estado_tareas}}</option>
								@foreach($todasdemas as $et)
